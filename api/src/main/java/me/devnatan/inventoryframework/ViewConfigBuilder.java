...
import me.devnatan.inventoryframework.exception.InvalidSizeException;
...
public final class ViewConfigBuilder {
	 ...
	 * @return This configuration builder.
	 * @throws InvalidSizeException If used after setting a layout and size differs from the layout
	 *                              length.
	 */
	public ViewConfigBuilder size(int size) {
		if (layout != null && size != layout.length) {
			throw new InvalidSizeException("The size differs from the set layout length.");
		}
	...
	public ViewConfigBuilder layout(String... layout) {
		if (this.size != 0 && layout.length != this.size) {
