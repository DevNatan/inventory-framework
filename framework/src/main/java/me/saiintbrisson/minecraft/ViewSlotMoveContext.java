 ...
 * moved from one place to another, either within the same container or between containers,
 * in which there is information about the item in which it was interacted if there is one.
 * <p>
 * The properties of the current context are the source definitions and the properties named with
...
public interface ViewSlotMoveContext extends ViewSlotClickContext {
	 ...
	 * <p>
	 * When this happens the target item goes to the sprite's cursor and the source item takes th
	...
	/**
	 * Whether the source item stacked with the target item. For this happen both items must
	 * be similar. You can get the stacking result by accessing the target container.
	 *
