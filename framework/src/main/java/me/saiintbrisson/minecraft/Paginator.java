 ...
 * <p>
 * {@link #source} is the raw paging data even if the data is not of the provided type (with factory).
 *
...
final class Paginator<T> {
	...
	Paginator(int pageSize, @NotNull Object source) {
		...
		if (source instanceof List)
			_source = (List<T>) source;
		else if (source instanceof Function)
			_factory = (Function<PaginatedViewContext<T>, List<T>>) source;
		else
			throw new IllegalArgumentException("Unsupported pagination source type: " + source.getClass().getName());
	...
	private CompletableFuture<List<T>> getPageLazy(@NotNull PaginatedViewContext<T> context) {
		...
		if (data instanceof List) {
			...
			return CompletableFuture.completedFuture(contents.isEmpty()
	 ...
	 * Accepts {@link List}, {@link Collection} and {@link Iterable}.
	 *
	...
	private static <T> List<T> toList(@NotNull Object value) {
		...
		if (value instanceof Iterable) {
			...
			for (final T item : (Iterable<? extends T>) value)
				list.add(item);
		...
		throw new IllegalArgumentException(String.format(
			"Failed to convert value to list: %s",
