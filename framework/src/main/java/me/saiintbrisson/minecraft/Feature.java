 ...
 * </p>
 * <p>
 * The recommended style of building a feature is to use a helper class as an instance value,
 * which will be taken when the Feature is referenced, and the class of the feature itself is private
 * while its key is public for later installation.
 * </p>
 * <h3>Configuration</h3>
 ...
 * for that there is the FeatureConfig, which is the first parameter of the Feature interface,
 * which must be a __configuration builder__ and not the configuration itself.
 * <pre><code>
 ...
 * It is important to note that features are singleton instances and can only be modified (by themselves)
 * once in their entire lifecycle, which is during their installation. The FeatureInstaller must
 * ensure that this is applied and the Feature instance can also add a second check to ensure that
 * there are not multiple references to a single feature.
 *
...
public interface Feature<C, R> {
	 ...
	 *
	 * @param platform  The feature installer platform.
