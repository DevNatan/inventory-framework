...
public abstract class AbstractVirtualView implements VirtualView {
	...
	void render(@NotNull ViewContext context) {
		...
		if (contextLayout != null || getLayout() != null) {
			...
			System.out.println("Rendering context layout");
	...
	final void renderLayout(
		...
		if (view.getReservedItems() == null || view.getReservedItems().isEmpty())
			return;

		final int layerSize = layoutItemsLayer.size();
		final int lastSlot = layoutItemsLayer.peek();

		System.out.println("[" + view.getClass().getSimpleName() + "] Layer size: " + layerSize + ", last slot: " + lastSlot);
		for (int i = 0; i < lastSlot; i++) {
			// check for items overflow
			if (i >= layerSize) break;

			final int targetSlot = layoutItemsLayer.elementAt(i);
			...
			try {
				...
				System.out.println("rendering slot at " + targetSlot + " of " + view.getClass().getSimpleName() + ": " + next);
	...
	protected final void render(@NotNull ViewContext context, @NotNull ViewItem item, int slot) {
		...
		if (item.getSlot() == ViewItem.AVAILABLE_SLOT)
			item.setSlot(slot);
