...
public final class NavigationControllerInterceptor implements PipelineInterceptor<ViewContext> {
	...
	private int getNavigationItemSlot(@NotNull PaginatedViewContext<?> context, int direction) {
		PaginatedVirtualView<?> target = context.isLayoutSignatureChecked()
			? context
			: context.getRoot();

		return direction == NAVIGATE_LEFT
			? target.getPreviousPageItemSlot()
			: target.getNextPageItemSlot();
	}
	...
	private <T> ViewItem createNavigationItemFromRoot(@NotNull PaginatedViewContext<T> context, int direction) {
		...
		final BiConsumer<PaginatedViewContext<T>, ViewItem> factory = direction == NAVIGATE_LEFT
			? root.getPreviousPageItemFactory()
			: root.getNextPageItemFactory();

		// TODO just a compatibility guarantee must be removed in v2.5.5
		if (factory == null) {
			@SuppressWarnings("deprecation") final ViewItem result = direction == NAVIGATE_LEFT
				? root.getPreviousPageItem(context)
				: root.getNextPageItem(context);
	...
	private <T> ViewItem resolveNavigationItem(
		...
		@SuppressWarnings("deprecation") final Function<PaginatedViewContext<?>, ViewItem> fallback = direction == NAVIGATE_LEFT
	...
	private <T> void updateNavigationItem(@NotNull PaginatedViewContext<T> context, int direction) {
		...
		final boolean notAvailable = item == null
			|| (
				item.getItem() == null
					&& item.getRenderHandler() == null
					&& item.getUpdateHandler() == null
		);
	...
	private void checkIndeterministicNavigationSlot(int layoutSlot, int itemSlot) {
		if (layoutSlot != UNSET || itemSlot != UNSET)
			return;
