...
public final class AvailableSlotRenderInterceptor implements PipelineInterceptor<VirtualView> {
	...
	public void intercept(
		...
		System.out.println("Rendering layout of " + view.getClass().getSimpleName() + "...");
		...
		if (contextLayout != null) {
			...
			System.out.println("Rendering context without layout");
			renderReservedItems(
				root,
				context,
	...
	private void renderReservedItems(
		...
		System.out.println("reserved items (targetingRoot: " + targetingRoot + "): " + reservedItems);
		...
		final boolean wasRootReservedItemsRendered = !targetingRoot
			&& root.isLayoutSignatureChecked()
			&& root.getReservedItems() != null;
		...
		if (wasRootReservedItemsRendered) {
			...
			layoutItemsLayer.addAll(!context.isLayoutSignatureChecked()
				? root.getLayoutItemsLayer()
		...
		System.out.println("Reserved (targetingRoot: " + targetingRoot + "): " + reservedItems);

		final int reservedItemsCount = reservedItems.size();
		System.out.println("Reserved items count: " + reservedItemsCount);
		...
		for (int i = offset; i < reservedItemsCount; i++) {
			...
			try {
				...
				throw new SlotFillExceededException(
					"No more slots available on layout to accommodate " + item,
					e
				);
			}
